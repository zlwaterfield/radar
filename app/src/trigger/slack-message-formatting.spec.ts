...
describe('Slack Message Formatting', () => {
  ...
  function createIssueSlackMessage(data: any) {
    ...
    const color = EVENT_COLORS[actionColorKey as keyof typeof EVENT_COLORS] || EVENT_COLORS.default;
  ...
  describe('Pull Request Messages', () => {
    it('should create correctly formatted PR opened message', () => {
      ...
      expect(titleBlock.text.text).toContain('<https://github.com/developer123|developer123>');
      ...
      expect(contentBlock.text.text).toContain('*PR #123* Fix critical bug in authentication');
    ...
    it('should use correct colors and icons for different PR actions', () => {
      const testCases = [
        ...
        { action: 'review_requested', expectedColor: '#ECB22E', expectedIcon: '👀' },
      ...
      testCases.forEach(({ action, expectedColor, expectedIcon }) => {
        ...
        expect(message.attachments[0].blocks[0].text.text).toContain(expectedIcon);
      });
    });
  });
  ...
  describe('Issue Messages', () => {
    it('should create correctly formatted issue opened message', () => {
      ...
      expect(titleBlock.text.text).toContain('<https://github.com/reporter789|reporter789>');

      // Verify content block
      const contentBlock = attachment.blocks[2];
      expect(contentBlock.text.text).toContain('*Issue #456* Database connection timeout');
      expect(contentBlock.text.text).toContain(data.url);
  ...
  describe('Message Structure Validation', () => {
    it('should always include required Slack Block Kit elements', () => {
      ...
      expect(message.attachments[0].blocks[0]).toHaveProperty('type', 'section');
      expect(message.attachments[0].blocks[0]).toHaveProperty('text');
      expect(message.attachments[0].blocks[0].text).toHaveProperty('type', 'mrkdwn');
      
      expect(message.attachments[0].blocks[1]).toHaveProperty('type', 'divider');
      
      expect(message.attachments[0].blocks[2]).toHaveProperty('accessory');
      expect(message.attachments[0].blocks[2].accessory).toHaveProperty('type', 'button');
    });
  });
});