...
export class LLMAnalyzerService {
   ...
   */
  async matchKeywordsWithLLM(content: string, keywords: string[]): Promise<{ matchedKeywords: string[]; matchDetails: Record<string, string> }> {
    this.logger.debug(`Matching keywords with LLM - Content length: ${content.length}, Keywords: ${keywords}`);
...
If no keywords match, return an empty list for matched_keywords.`;
      ...
      this.logger.debug(`LLM request: content length ${content.length}, keywords: ${keywords}`);
      ...
      if (!response.ok) {
        this.logger.error(`LLM API ERROR: Status ${response.status}, Response: ${await response.text()}`);
        return { matchedKeywords: [], matchDetails: {} };
      }
      ...
      this.logger.debug(`LLM response received: ${responseContent?.length || 0} characters`);
      ...
      try {
        ...
        this.logger.log(`LLM keyword matching results - Matched: ${matchedKeywords}, Details: ${JSON.stringify(matchDetails)}`);
   ...
   */
  async analyzeContent(content: string, userId: string): Promise<{ shouldNotify: boolean; matchedKeywords: string[]; matchDetails: Record<string, string> }> {
    try {
      this.logger.debug(`Analyzing content for user ${userId} - Content length: ${content.length}`);
      ...
      this.logger.log(`Keyword check for user ${userId}: enabled=${keywordNotificationsEnabled}, keywords=${keywords.length}, threshold=${threshold}`);
      ...
      const { matchedKeywords, matchDetails } = await this.matchKeywordsWithLLM(content, keywords);
      
      // Determine if notification should be sent
      const shouldNotify = matchedKeywords.length > 0;
      
      this.logger.log(`Content analysis complete for user ${userId} - Should notify: ${shouldNotify}, Matched keywords: ${matchedKeywords}`);
