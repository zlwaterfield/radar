...
export class NotificationsController {
  ...
  @Post('process')
  @ApiOperation({
    summary: 'Manually trigger notification processing for current user',
  })
  @ApiResponse({ status: 200, description: 'Notifications processed' })
  async processNotifications(@GetUser() user: User) {
    this.logger.log(
      `Manual notification processing triggered for user ${user.id}`,
    );

    const result =
      await this.distributionService.processUserNotificationsManually(user.id);
   ...
   */
  private async getRecentSentNotifications(
    userId: string,
    limit = 25,
  ): Promise<any[]> {
    try {
      return await this.distributionService[
        'databaseService'
      ...
      this.logger.error(
        `Error getting recent sent notifications for user ${userId}:`,
        error,
      );
      return [];
    }
