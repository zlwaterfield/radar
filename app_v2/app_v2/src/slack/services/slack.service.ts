...
export class SlackService {
  ...
  async sendMessage(
    ...
    try {
      const client = accessToken ? this.createUserClient(accessToken) : this.botClient;
   ...
   * Get Slack user info
   */
  async getUserInfo(userId: string, accessToken?: string): Promise<SlackUser | null> {
    try {
      const client = accessToken ? this.createUserClient(accessToken) : this.botClient;
   ...
   */
  async openDMChannel(userId: string, accessToken?: string): Promise<string | null> {
    try {
      const client = accessToken ? this.createUserClient(accessToken) : this.botClient;
  ...
  private setupSlackEventHandlers(): void {
    ...
    this.slackApp.event('app_home_opened', async ({ event, logger }) => {
      try {
    ...
    this.slackApp.event('message', async ({ message, say }) => {
      try {
    ...
    this.slackApp.command('/radar', async ({ command, ack, respond }) => {
      await ack();
  ...
  private async handleAppHomeOpened(userId: string): Promise<void> {
    try {
      ...
      const blocks = user ? this.createAuthenticatedHomeView(user) : this.createUnauthenticatedHomeView();

      await this.publishHomeView(userId, blocks);
    } catch (error) {
      this.logger.error(`Error handling app home opened for user ${userId}:`, error);
    }
  }
