...
export class GitHubService {
  ...
  async createInstallationClient(installationId: number): Promise<Octokit> {
    ...
    try {
      const { data: installation } = await appClient.apps.createInstallationAccessToken({
   ...
   */
  async getUserRepositories(userId: string, includePrivate = true): Promise<GitHubRepository[]> {
    try {
      ...
      const decryptedToken = this.tokenService.decryptExternalToken(user.githubAccessToken);
      const octokit = this.createUserClient(decryptedToken);
      ...
      this.logger.log(`Retrieved ${repositories.length} repositories for user ${userId}`);
      return repositories as any[];
    } catch (error) {
      this.logger.error(`Error fetching repositories for user ${userId}:`, error);
      throw error;
    }
  }
  ...
  async getPullRequests(
    ...
    try {
      const octokit = accessToken ? this.createUserClient(accessToken) : this.createAppClient();
      ...
      this.logger.error(`Error fetching pull requests for ${owner}/${repo}:`, error);
      throw error;
    }
  }
  ...
  async getPullRequest(
    ...
    try {
      ...
      this.logger.error(`Error fetching PR #${pullNumber} for ${owner}/${repo}:`, error);
      throw error;
    }
  }
   ...
   */
  async getUserInstallations(accessToken: string): Promise<GitHubInstallation[]> {
    try {
      const octokit = this.createUserClient(accessToken);
  ...
  async getInstallationRepositories(installationId: number): Promise<GitHubRepository[]> {
    try {
      ...
      this.logger.error(`Error fetching repositories for installation ${installationId}:`, error);
      throw error;
    }
  }
  ...
  async testConnection(accessToken: string): Promise<boolean> {
    try {
      ...
      this.logger.debug('GitHub connection test failed:', error.message);
      return false;
    }
  }
  ...
  private getPrivateKey(): string {
    ...
    if (privateKeyPath) {
      ...
      try {
        ...
        this.logger.error(`Failed to read private key from ${privateKeyPath}:`, error);
        throw error;
      }
    }
