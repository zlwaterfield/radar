...
export class GitHubController {
  ...
  async getRepositories(
    ...
    return this.githubService.getUserRepositories(user.id, includePrivate !== false);
  }
  ...
  @ApiResponse({ status: 200, description: 'List of installation repositories' })
  async getInstallationRepositories(@Param('installationId') installationId: string) {
  ...
  @ApiQuery({ name: 'repositories', description: 'Comma-separated list of repository full names' })
  @ApiResponse({ status: 200, description: 'Repositories connected successfully' })
  async connectRepositories(
    ...
    const repositoryNames = repositoriesParam.split(',').map(name => name.trim());
    const results = [];
    ...
    for (const fullName of repositoryNames) {
      ...
      try {
        ...
        const repo = await this.githubService.getRepository(parsed.owner, parsed.repo, user.githubAccessToken);
        ...
        if (existing) {
          ...
          results.push({ repository: fullName, status: 'updated', id: updated.id });
        } else {
          ...
          results.push({ repository: fullName, status: 'connected', id: created.id });
        }
  ...
  async toggleRepository(
    ...
    if (!repository) {
      throw new BadRequestException('Repository not found or not owned by user');
    }
  ...
  async getPullRequests(
    ...
    return this.githubService.getPullRequests(owner, repo, state || 'all', user.githubAccessToken);
  }
  ...
  async getIssues(
    ...
    return this.githubService.getIssues(owner, repo, state || 'all', user.githubAccessToken);
  }
  ...
  async testConnection(@CurrentUser() user: any) {
    ...
    const isConnected = await this.githubService.testConnection(user.githubAccessToken);
