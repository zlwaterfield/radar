...
import type { NotificationData, SlackMessage, SlackBlock } from '@/common/types';
...
export class NotificationDistributionService {
  ...
  async processNotifications(): Promise<void> {
    ...
    try {
      ...
      const usersWithNotifications = await this.getUsersWithPendingNotifications();
  ...
  async processUserNotifications(userId: string): Promise<void> {
    try {
      ...
      const notifications = await this.notificationsService.getPendingNotifications(userId, 10);
      
      if (notifications.length === 0) {
        return;
      }

      this.logger.log(`Processing ${notifications.length} notifications for user ${userId}`);

      // Open DM channel with user
      const channelId = await this.slackService.openDMChannel(user.slackId, user.slackAccessToken);
      ...
      this.logger.error(`Error processing notifications for user ${userId}:`, error);
    }
  }
  ...
  private async sendNotification(
    ...
    try {
      ...
      const result = await this.slackService.sendMessage(slackMessage, user.slackAccessToken);
      ...
      if (result) {
        ...
        this.logger.log(`Sent notification ${notification.id} to user ${user.id}`);
      } else {
        this.logger.error(`Failed to send notification ${notification.id} to user ${user.id}`);
      }
    } catch (error) {
  ...
  private async getUsersWithPendingNotifications(): Promise<string[]> {
    try {
      ...
      this.logger.error('Error getting users with pending notifications:', error);
      return [];
    }
  }
  ...
  async processUserNotificationsManually(userId: string): Promise<{
    ...
    try {
      const pendingBefore = await this.notificationsService.getPendingNotifications(userId);
      await this.processUserNotifications(userId);
      const pendingAfter = await this.notificationsService.getPendingNotifications(userId);
      ...
      this.logger.error(`Error manually processing notifications for user ${userId}:`, error);
      return {
        success: false,
        processedCount: 0,
        error: error.message,
