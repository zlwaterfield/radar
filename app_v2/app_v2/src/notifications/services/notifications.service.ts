...
export class NotificationsService {
  ...
  async processEvent(event: any): Promise<boolean> {
    try {
      ...
      const relevantUsers = await this.getRelevantUsers(repositoryName, payload);

      if (relevantUsers.length === 0) {
        this.logger.debug(`No relevant users found for ${eventType} in ${repositoryName}`);
        return true;
      }
      ...
      for (const user of relevantUsers) {
        const shouldNotify = await this.shouldNotifyUser(user, eventType, action, payload);
      ...
      this.logger.log(`Created ${notifications.length} notifications for ${eventType} in ${repositoryName}`);
      return true;
    } catch (error) {
   ...
   */
  private async getRelevantUsers(repositoryName: string, payload: any): Promise<any[]> {
    try {
  ...
  private async shouldNotifyUser(
    ...
    try {
      const preferences = await this.userSettingsService.getNotificationPreferences(user.id);
      const schedule = await this.userSettingsService.getNotificationSchedule(user.id);
      ...
      this.logger.error(`Error checking notification preferences for user ${user.id}:`, error);
      return false;
    }
  }
  ...
  private async createNotification(
    ...
    try {
      ...
      const messageData = this.createNotificationData(eventType, action, payload, repositoryName);
    ...
    try {
      switch (eventType) {
        ...
        case 'pull_request_review':
          return this.createPRReviewNotificationData(action, payload, repositoryName);
        
        case 'issues':
          return this.createIssueNotificationData(action, payload, repositoryName);
        
        case 'issue_comment':
          return this.createIssueCommentNotificationData(action, payload, repositoryName);
        
        case 'pull_request_review_comment':
          return this.createPRCommentNotificationData(action, payload, repositoryName);
        
        default:
          return null;
      }
    } catch (error) {
      this.logger.error(`Error creating notification data for ${eventType}:`, error);
      return null;
    }
  }
   ...
   */
  private getEventPreferenceKey(eventType: string, action: string): string | null {
  ...
  async getPendingNotifications(userId: string, limit = 50): Promise<any[]> {
    try {
      ...
      this.logger.error(`Error getting pending notifications for user ${userId}:`, error);
      return [];
    }
  }
  ...
  async markNotificationSent(
    ...
    try {
      ...
      this.logger.error(`Error marking notification ${notificationId} as sent:`, error);
      return false;
    }
  }
}