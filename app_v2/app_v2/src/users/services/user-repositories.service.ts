...
export class UserRepositoriesService {
  ...
  async getUserRepositories(userId: string): Promise<UserRepository[]> {
    try {
      ...
      this.logger.error(`Error getting repositories for user ${userId}:`, error);
      return [];
    }
  }
   ...
   */
  async addUserRepository(userId: string, repositoryData: {
    githubId: string;
    name: string;
    fullName: string;
    description?: string;
    ...
    try {
      ...
      this.logger.log(`Added repository ${repositoryData.fullName} for user ${userId}`);
      return repository;
    } catch (error) {
   ...
   */
  async updateRepository(userId: string, repositoryId: string, data: {
    enabled?: boolean;
    isActive?: boolean;
  }): Promise<UserRepository> {
    try {
   ...
   */
  async syncUserRepositories(userId: string, githubAccessToken: string): Promise<{
    added: number;
    updated: number;
    total: number;
  }> {
    try {
      ...
      const githubRepos = await this.githubService.getUserRepositories(githubAccessToken);
      ...
      this.logger.log(`Synced repositories for user ${userId}: ${added} added, ${updated} updated`);
      ...
      this.logger.error(`Error syncing repositories for user ${userId}:`, error);
      throw error;
    }
  }
  ...
  async getRepositoryStats(userId: string): Promise<{
    ...
    try {
      ...
      this.logger.error(`Error getting repository stats for user ${userId}:`, error);
      return {
        total: 0,
        enabled: 0,
        private: 0,
  ...
  async getEnabledRepositories(userId: string): Promise<UserRepository[]> {
    try {
      ...
      this.logger.error(`Error getting enabled repositories for user ${userId}:`, error);
      return [];
    }
  }
}