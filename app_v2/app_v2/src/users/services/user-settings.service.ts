...
export class UserSettingsService {
   ...
   */
  async createUserSettings(userId: string, data: CreateUserSettingsDto): Promise<UserSettings> {
    try {
  ...
  async getNotificationPreferences(userId: string): Promise<any> {
    ...
    return settings?.notificationPreferences || this.getDefaultNotificationPreferences();
  }
  ...
  async getNotificationSchedule(userId: string): Promise<any> {
    const settings = await this.getUserSettings(userId);
    return settings?.notificationSchedule || this.getDefaultNotificationSchedule();
  }
   ...
   * Update keywords
   */
  async updateKeywords(userId: string, keywords: string[]): Promise<UserSettings> {
    const existingSettings = await this.getUserSettings(userId);
   ...
   */
  async updateStatsTimeWindow(userId: string, timeWindow: number): Promise<UserSettings> {
    const existingSettings = await this.getUserSettings(userId);
