...
export class AuthService {
  ...
  async createOrUpdateUser(providerData: {
    ...
    const { providerId, accountId, email, name, image, accessToken, refreshToken } = providerData;
    ...
    try {
      ...
      if (existingAccount) {
        ...
        await this.databaseService.account.update({
          ...
          data: {
            accessToken: accessToken ? this.encryptToken(accessToken) : existingAccount.accessToken,
            refreshToken: refreshToken ? this.encryptToken(refreshToken) : existingAccount.refreshToken,
            updatedAt: new Date(),
          },
        });
      } else {
        ...
        if (providerId === 'slack') {
          Object.assign(userData, {
            ...
            slackRefreshToken: refreshToken ? this.encryptToken(refreshToken) : null,
          });
          ...
          Object.assign(userData, {
            ...
            githubId: accountId,
            githubLogin: name || '',
            githubAccessToken: accessToken ? this.encryptToken(accessToken) : null,
            githubRefreshToken: refreshToken ? this.encryptToken(refreshToken) : null,
          });
        }
      ...
      this.logger.error(`Error creating/updating user from ${providerId}:`, error);
      throw error;
    }
  }
  ...
  async getUserWithTokens(userId: string, decryptTokens = false) {
    ...
    return {
      ...user,
      slackAccessToken: user.slackAccessToken ? this.decryptToken(user.slackAccessToken) : null,
      slackRefreshToken: user.slackRefreshToken ? this.decryptToken(user.slackRefreshToken) : null,
      githubAccessToken: user.githubAccessToken ? this.decryptToken(user.githubAccessToken) : null,
      githubRefreshToken: user.githubRefreshToken ? this.decryptToken(user.githubRefreshToken) : null,
    };
  }
