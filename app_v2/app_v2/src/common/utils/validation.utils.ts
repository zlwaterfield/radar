...
export function extractMentionedUsernames(text: string): string[] {
  ...
  const mentions = text.match(/(?:^|[^a-zA-Z0-9.])@([a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?)/g);
...
export function validateNotificationPreferences(preferences: any): boolean {
  ...
  const validKeys = [
    ...
    'pull_request_reviewed', 'pull_request_commented', 'pull_request_assigned',
    'issue_opened', 'issue_closed', 'issue_commented', 'issue_assigned',
    'discussion_created', 'discussion_answered', 'discussion_commented',
    'push_to_branch', 'mention_in_comment', 'mention_in_pr', 'mention_in_issue'
  ];

  for (const [key, value] of Object.entries(preferences)) {
    if (!validKeys.includes(key) || (value !== undefined && typeof value !== 'boolean')) {
      return false;
    }
  }
