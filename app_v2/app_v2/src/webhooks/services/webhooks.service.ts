...
export class WebhooksService {
  ...
  async processGitHubWebhook(
    ...
    try {
      ...
      this.logger.log(`Processed ${eventType} event for repository ${payload.repository?.name}`);
      return true;
    } catch (error) {
      this.logger.error(`Error processing GitHub webhook (${eventType}):`, error);
      return false;
    }
  }
   ...
   */
  async getEventsByRepository(repositoryId: string, limit = 20): Promise<any[]> {
    try {
      return await this.databaseService.event.findMany({
        where: {
          repositoryId,
      ...
      this.logger.error(`Error getting events for repository ${repositoryId}:`, error);
      return [];
    }
  }
